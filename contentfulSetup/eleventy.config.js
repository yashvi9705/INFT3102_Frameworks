...
async function createEmployee(employee) {
  try {
    ...
    const maleAssetId = process.env.MALE_ASSET_ID; // Replace with actual Asset ID for male
    const femaleAssetId = process.env.FEMALE_ASSET_ID;

    const imageAssetId = (employee.gender || "").toUpperCase() === "F" ? femaleAssetId : maleAssetId;
...
async function CmsHelper(contentType, dateField){
  try {
    ...
    // Log the entries to see the structure 
